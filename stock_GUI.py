...
class StockApp:
    def __init__(self):
        ...
        # Create Window
        self.root = Tk()
        self.root.title("Michelle's Finance Watcher")
        self.root.geometry("500x550")

        #Colorize the GUI
        self.style = ttk.Style()
        self.style.configure('TFrame', background='#bdb9b9')
        self.style.configure('TLabel', background='#bdb9b9')
        self.style.configure('TNootebook', background="#bdb9b9")
        self.style.configure('TButton', background='#535353')
        ...
        self.headingLabel = Label(self.root,text="No Stock Selected", font=("Serif", 14))
        self.headingLabel.grid(column=0,row=0,columnspan=3,padx = 5, pady = 10)
        

        # Add stock list
        self.stockLabel = Label(self.root,text="Stocks", font=("Mono", 10))
        self.stockLabel.grid(column=0,row=1,padx = 5, pady = 10,sticky=(N))

        self.stockList = Listbox(self.root, height=10)
        ...
        # Set Up Main Tab
        self.addStockGroup = LabelFrame(self.mainFrame,text="Add Stock",padx=5,pady=5, bg="#bdb9b9", font=("Serif", 12))
        self.addStockGroup.grid(column=0,row=0,padx=5,pady=5,sticky=(W,E))
     
        self.addSymbolLabel = Label(self.addStockGroup,text = "Symbol", bg="#bdb9b9", font=("Mono", 10))
        ...
        self.addNameLabel = Label(self.addStockGroup,text = "Name", bg="#bdb9b9", font=("Mono", 10))
        ...
        self.addSharesLabel = Label(self.addStockGroup,text = "Shares", bg="#bdb9b9", font=("Mono", 10))
        ...
        self.addStockButton = Button(self.addStockGroup,text = "New Stock", font=("Mono", 8),command=self.add_stock)
        self.addStockButton.grid(column=0,row=3,columnspan = 2, padx = 5, pady = 5)


        self.deleteGroup = LabelFrame(self.mainFrame,text="Delete Stock",padx=5,pady=5, bg="#bdb9b9", font=("Serif", 12))
        self.deleteGroup.grid(column=0,row=2,padx=5,pady=5,sticky=(W,E))

        self.deleteStockButton = Button(self.deleteGroup,text="Delete Selected Stock", font=("Mono", 8),command=self.delete_stock)
        self.deleteStockButton.grid(column=0,row=0,columnspan = 2, padx = 5, pady = 5)
    ...
    def delete_stock(self):
        #Step 1: Check Stock is Selected
        if self.stockList.curselection():
            symbol = self.stockList.get(self.stockList.curselection())

            #Going to Loop to find stock with selected Symbol
            ...
            #Found the Stock! Lets Delete it
            if stock_to_delete:
                self.stock_list.remove(stock_to_delete)
